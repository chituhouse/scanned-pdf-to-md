...
## Claude Code Skills 方法论

### 什么是 Skill？

Skill 是 Claude Code 的自定义命令系统。通过在 `.claude/skills/` 目录下定义 Markdown 文件，可以让 Claude 按照预设的流程执行复杂任务。
...
**调用方式**：在 Claude Code 中输入 `/skill-name`，如 `/book-ocr`

### 为什么用 Skill 而不是纯 Python？

| 方式 | 优势 | 劣势 |
|:---|:---|:---|
| 纯 Python | 执行稳定、可复现 | 难以处理语义判断、边界情况需要硬编码 |
| 纯 LLM | 灵活、能理解语义 | 不稳定、容易"发挥" |
| **Skill（结合两者）** | **Python 处理确定性任务 + LLM 处理语义判断** | 需要设计好任务边界 |

**核心洞察**：OCR 后处理中，80% 的工作是确定性的（格式转换、文件操作），20% 需要语义理解（判断内容完整性、处理歧义）。Skill 让 LLM 专注于那 20%。

### Skill 设计原则

#### 1. 铁律约束 —— 防止 AI 发挥

LLM 有"创造性"，在 OCR 场景中这是 bug 不是 feature。

```markdown
## 铁律（必须遵守）
1. **不发挥、不编造**：只使用源文档中已有的内容
2. **不猜测答案**：缺失内容标注 `[缺失]`，不编造
3. **不修改原文**：只修复格式，不改动文字本身
```

**为什么有效**：明确告诉 LLM "什么不能做"，比"什么能做"更重要。

#### 2. 逐步确认 —— 防止批量失控

```markdown
**重要：必须一套一套处理，绝对不能批量处理！**
**每套处理完必须展示报告给用户确认！**
```

**教训**：项目初期尝试批量处理 20 套题，后期质量明显下降。LLM 在长任务中会"疲劳"。
...
| 特性 | `/cross-validate-ocr` | `/book-ocr` |
|:---|:---|:---|
| 用途 | 题库类文档（有标准结构） | 书籍类文档（自由格式） |
| 输入 | 两个 MD 文件（双路 OCR 结果） | 图片目录 |
| 输出 | 验证后的 MD 文件 | Markdown + Word/ePub/PDF |
| 核心逻辑 | 逐题对比合并 | 双路 OCR + 图片提取 |
| 用户交互 | 每套题确认 | 执行前确认格式和计划 |
